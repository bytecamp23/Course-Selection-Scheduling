...
- 张伟明(1zwm)负责登录和成员的接口、压力测试。
- 邓泽晖(nArrow4)负责排课模块（排课求解器除外）、请求参数校验器。
- 刘曜辉(AsakusaRinne/Yaohui Liu)负责成员和排课的部分接口。
- 杨志刚(LMessi2000)LMessi2000负责文档编写、基础架构、排课求解器、代码重构测试。
- 曾林藩(LinZengCn)负责抢课模块。
